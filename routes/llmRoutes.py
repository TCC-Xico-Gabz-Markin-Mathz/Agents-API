...
from services.llm import get_sql_query_with_database_structure, get_result_interpretation
...
async def query_interpreter(request: InterpreterQueryRequest):
    try:
        response = await get_result_interpretation(
            result=request.result,
            order=request.order
        )
        return InterpreterQueryResponse(response=response)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao processar Interpretador: {str(e)}")